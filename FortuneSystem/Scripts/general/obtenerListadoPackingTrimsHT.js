		...
		success: function (jsonData) {
			...
			if (listaPBulk.length === 0 && listaEPPK.length === 0) {
				...
				if (cargo === 1 || cargo === 10) {
					...
					$("#div_titulo_Register").html("<h3>PALLET INFORMATION</h3>");
...
function RegistrarEmpaqueBulkHT(nPO, tEmpaque) {
	...
	$("#tablaTallasBulkHT").hide();
	$("#tablaTallasPPKHT").hide();
	$("#opcionesRegistroHTPPK").css("display", "none");
			...
			if (listaPackingBox.length !== 0) {
				...
				$.each(listaPackingBox, function (key, item) {
					...
					if (tallaTemp === item.Talla) {
						...
						html += '<td width="20%"><input type="text" name="l-cantidadPcs" id="l-cantidadPcs" class="form-control numeric qtyPcs " value="' + cantTempPart + '"  readonly/></td>';
...
function RegistrarEmpaquePPKHT(nPO, tEmpaque) {
	$("#tablaTallasBulkHT").hide();
	$("#opcionesRegistroHTPPK").css("display", "inline");
...
function limpiarFormPPK() {
	...
	//$('#Packing_PackingTypeSize_FormaEmpaque').val(0)
	$('#Packing_PackingTypeSize_NumberPO').val('');
	$("#Packing_PackingTypeSize_TotalUnits").val('');
}
...
$(document).on("click", "#nuevoAddEmpaqueBulkHT", function () {
	var r = 0; var c = 0; var i = 0; var cadena = new Array(5);
	cadena[0] = ''; cadena[1] = ''; cadena[2] = ''; cadena[3] = ''; cadena[4] = '';
	var nFilas = $("#tablaTallasAddBulkHT tbody>tr").length;
	var nColumnas = $("#tablaTallasAddBulkHT tr:last td").length;
	$('#tablaTallasAddBulkHT tbody>tr').each(function () {
		r = 0;
		c = 0;
		$(this).find("input").each(function () {
			$(this).closest('td').find("input").each(function () {
	...
	var error = 0;
	$('#tablaTallasAddBulkHT').find('td').each(function (i, el) {
		var valor = $(el).children().val();
		if ($(el).children().val() === '') {
			error++;
			$(el).children().css('border', '2px solid #e03f3f');
	...
	var formaEmpaque = $("#TipoFormaEmpaque option:selected").val();
	if (formaEmpaque === "0") {
		error++;
		$('#TipoFormaEmpaque').css('border', '2px solid #e03f3f');
	}
	else {
		$('#TipoFormaEmpaque').css('border', '');
	}

	var numberPO = $("#numeroPOPackActBulkHTP").val();
	if (numberPO === "") {
		error++;
		$('#numeroPOPackActBulkHTP').css('border', '2px solid #e03f3f');
	}
	else {
		$('#numeroPOPackActBulkHTP').css('border', '');
	}

	enviarListaTallaAddBulkHT(cadena, error);
});
//}


function enviarListaTallaAddBulkHT(cadena, error) {
	var idFormaP = $("#TipoFormaEmpaque option:selected").val();
	var idNumberPo = $("#numeroPOPackActBulkHTP").val();
	if (error !== 0) {
		var alert = alertify.alert("Message", 'All fields are required.').set('label', 'Aceptar');
		alert.set({ transition: 'zoom' });
		alert.set('modal', false);
		...
		$.ajax({
			...
			success: function (data) {
				alertify.set('notifier', 'position', 'top-right');
				alertify.notify('The type of packaging is registered correctly.', 'success', 5, null);
				$("#btnAddNuevo").prop("disabled", false);
				$("#btnAddNext").prop("disabled", false);
				$("#btnAddDone").prop("disabled", false);
				$("#nuevoAddEmpaqueBulkHT").prop("disabled", true);
				// obtenerListaTallas(estiloId);
			},
			error: function (xhr, ajaxOptions, thrownError) {
...
$(document).on("click", "#nuevoAddEmpaquePPKHT", function () {
	var r = 0; var c = 0; var i = 0; var cadena = new Array(2);
	cadena[0] = ''; cadena[1] = '';
	var nFilas = $("#tablaTallasAddPPKHT tbody>tr").length;
	var nColumnas = $("#tablaTallasAddPPKHT tr:last td").length;
	$('#tablaTallasAddPPKHT tbody>tr').each(function () {
		r = 0;
		c = 0;
		$(this).find("input").each(function () {
			$(this).closest('td').find("input").each(function () {
	...
	var error = 0;
	$('#tablaTallasAddPPKHT').find('td').each(function (i, el) {
		var valor = $(el).children().val();
		if ($(el).children().val() === '') {
			error++;
			$(el).children().css('border', '2px solid #e03f3f');
	...
	var numberPO = $("#numeroPOPackActPPKHTP").val();
	if (numberPO === "") {
		error++;
		$('#numeroPOPackActPPKHTP').css('border', '2px solid #e03f3f');
	}
	else {
		$('#numeroPOPackActPPKHTP').css('border', '');
	}

	var tUnits = $("#numTotalUnitPPK").val();
	if (tUnits === "") {
		error++;
		$('#numTotalUnitPPK').css('border', '2px solid #e03f3f');
	}
	else {
		$('#numTotalUnitPPK').css('border', '');
	}
	enviarListaTallaAddPPKHT(cadena, error);
});


function enviarListaTallaAddPPKHT(cadena, error) {
	var idNumberPo = $("#numeroPOPackActPPKHTP").val();
	var totalUnits = $("#numTotalUnitPPK").val();
	if (error !== 0) {
		var alert = alertify.alert("Message", 'All fields are required.').set('label', 'Aceptar');
		alert.set({ transition: 'zoom' });
		alert.set('modal', false);
		...
		$.ajax({
			...
			success: function (data) {
				...
				$("#nuevoAddEmpaquePPKHT").prop("disabled", true);
				//obtenerListaTallas(estiloId);
			},
			error: function (xhr, ajaxOptions, thrownError) {
...
function TallasEmpaquePPKHT(idEst) {
	...
	$.ajax({
		...
		success: function (jsonData) {
			...
			$('#opcionesPPKPack').css("display", "none");			
			$('label[id="numTotalUnitLabel"]').hide();
			$("#numTotalUnit").hide();
...
$(document).on("click", "#nuevoEmpaquePPKHT", function () {
	...
	var tUnits = $("#Packing_PackingTypeSize_TotalUnitsPPKHT").val();
	if (tUnits === "") {
		error++;
		$('#Packing_PackingTypeSize_TotalUnitsPPKHT').css('border', '2px solid #e03f3f');
	}
	else {
		$('#Packing_PackingTypeSize_TotalUnitsPPKHT').css('border', '');
	}
...
function enviarListaTallaPPKHT(cadena, error) {
	var idNumberPo = $("#Packing_PackingTypeSize_NumberPO").val();
	var totalUnits = $("#Packing_PackingTypeSize_TotalUnitsPPKHT").val();
	if (error !== 0) {
		var alert = alertify.alert("Message", 'All fields are required.').set('label', 'Aceptar');
		alert.set({ transition: 'zoom' });
		alert.set('modal', false);
...
$(function () {
	...
	$('#TipoEmpPackAdd').change(function () {
		var selectedText = $(this).find("option:selected").text();
		var selectedValue = $(this).val();
		var html = '';		
		var tEmpaque = $("#TipoEmpPackAdd option:selected").val();

		if (selectedValue === "1") {
			$("#grupoBotonesBulkAdd").css("display", "none");
			$("#tablaTallasAddBulkHT").css("display", "inline");
			$("#TipoFormaEmpaque").css("display", "inline");
			$("#opcionesNumPO").css("display", "inline");
			$("#opcionestotalPPK").css("display", "none");
			
			$('label[for="Packing_CantBox"]').hide();
			$("#numeroCajas").hide();
			$('label[for="Packing_CantidadPPKS"]').hide();
			$("#Packing_CantidadPPKS").hide();
			$('label[id="Packing_PackingTypeSize_TotalUnitsPPKActHT"]').hide();
			$("#Packing_PackingTypeSize_TotalUnitsPPKActHT").hide();		
			TallasEmpaqueAddBulkHT(estiloId);


		} else if (selectedValue === "2") {
			$("#tablaTallasAddPPKHT").css("display", "inline");		
			$("#opcionestotalPPK").css("display", "inline");
			$('label[id="numTotalUnitLabelPPK"]').show();
			$("#numTotalUnitPPK").show();
			$("#numTotalUnitPPK").val(0);
			TallasEmpaqueAddPPKHT(estiloId);
		}
	});
});
...
$(document).on('change', '#numeroPOPackHTP', function () {
	var npo = $("#numeroPOPackHTP").val();
	if (npo === "") {
		npo = 0;
	}

	var tEmpaque = $("#TipoEmpPack option:selected").val();
	if (tEmpaque === "1") {
		ActualizarEmpaqueBulkHT(npo, tEmpaque);
	} else if (tEmpaque === "2") {
		
		$("#opcionesPPKPack").css('display', 'inline');
		ActualizarEmpaquePPKHT(npo, tEmpaque);
	}
